...
import { combineOpenClawIndexShards, importOpenClawSource, validateOpenClawInventory } from './openclaw.mjs';
...
export async function validateInventory(root) {
  ...
  const sourceImport = importOpenClawSource(catalogCsv, schemaCsv);
  const indexShards = await readJsonDirectory(path.join(root, 'inventory', 'external', 'openclaw', 'index'));
  const openclaw = {
    catalog: sourceImport.catalog,
    index_shards: indexShards,
    index: combineOpenClawIndexShards(indexShards),
  ...
  if (JSON.stringify(openclaw.index) !== JSON.stringify(sourceImport.index)) errors.push('OpenClaw discovery index drifted from preserved sources');
  if (JSON.stringify(openclaw.schema) !== JSON.stringify(sourceImport.schema)) errors.push('OpenClaw normalized schema drifted from preserved sources');
  if (JSON.stringify(openclaw.summary) !== JSON.stringify(sourceImport.summary)) errors.push('OpenClaw summary drifted from preserved sources');
